          ...
          {usuario.map((usuario) => (
            ...
            >
              {editandoID === usuario.id ? (
                ...
                <div>
                  <p className="text-lg font-semibold text-purple-700">{usuario.nombre}</p>
                  <p className="text-gray-600">{usuario.descripcion}</p>
                  <p className="text-sm text-gray-500">{usuario.correo}</p>
                  <p className="text-sm text-gray-500 mb-3">{usuario.celular}</p>
                  <div className="flex justify-end space-x-3">
                    <button
                      ...
                      {editandoID == tarea.id ? (
                            <>
                                ...
                                <input 
                                    ...
                                    onChange={(e)=>setDescripcionEditada(e.target.value)}
                                />
                                <button className="bg-purple-400 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200"  onClick={() => editarTarea(tarea.id)}>Guardar</button>
                                <button className="bg-purple-400 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200" onClick={() => setEditandoID(null)}>Cancelar</button>
                            </>
                        ) : (
                            <>
                            <p>{tarea.nombre} - {tarea.descripcion}</p>
                            <div className="flex justify-start space-x-2 m-3">
                            <button  className="bg-purple-400 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200"  onClick={() =>{
                                    setEditandoID(tarea.id);
                                    setNombreEditado(tarea.nombre);
                                    setDescripcionEditada(tarea.descripcion);
                                } }>Editar</button>
                                <button className="bg-purple-400 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-200" onClick={() => eliminarTarea(tarea.id)}>Eliminar</button>
                            </div>
                            </>
